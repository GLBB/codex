...
触发入口在 `run_pre_sampling_compact` 和 turn 中途的 `run_auto_compact`。核心判断来自 `auto_compact_token_status`。
...
```mermaid
flowchart TD
    A["turn before sampling / mid-turn"] --> B["auto_compact_token_status"]
...
- `auto_compact_token_status` 同时看 full active context 和 configured auto-compact scope。
