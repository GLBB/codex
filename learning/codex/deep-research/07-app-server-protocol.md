...
| `codex-rs/app-server-protocol/src/protocol/v2.rs` | v2 协议类型。 |
...
| `ThreadRequestProcessor` | `codex-rs/app-server/src/thread_request_processor.rs` | `thread/start`、`thread/resume`、`thread/list` 等 thread 级 RPC。 |
| `TurnRequestProcessor` | `codex-rs/app-server/src/turn_request_processor.rs` | `turn/start`、interrupt、approval response 等 turn 级 RPC。 |
...
| `protocol/v2.rs` | `codex-rs/app-server-protocol/src/protocol/v2.rs` | v2 wire contract，Rust 类型和 TypeScript schema 的来源。 |
