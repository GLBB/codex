...
should_defer =
    ...
    and (
        ...
        or deferred_candidates.len() >= DIRECT_MCP_TOOL_EXPOSURE_THRESHOLD // 100
    )
...
这就是 tool_search 的上下文预算逻辑：工具少时直接把 schema 给模型，工具多时先只给 `tool_search`，让模型按查询加载相关工具。
...
flowchart TD
    ...
    F --> G
    G --> H{tool_search enabled and count >= 100 or always defer?}
