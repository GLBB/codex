...
用户要求：“把测试环境网站标题改成‘夏日促销’。”

修改文件前，Agent 需要确认当前分支和文件版本正确；修改之后，不能只看工具返回 `success`，还要重新读取文件，确认标题真的变了，并确认没有改动无关内容。

```text
Preconditions
    当前分支正确，文件版本没有变化，修改范围合法
        ↓
Action
    修改网站标题
        ↓
...
这个检查只能证明“修改文件”这一步成功，不能证明网站已经部署，也不能证明用户最终看到的页面正确。
